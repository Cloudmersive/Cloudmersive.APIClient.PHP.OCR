...
> \Swagger\Client\Model\PdfToTextResponse pdfOcrGetAsyncJobStatus($async_job_id)
...
try {
    $result = $apiInstance->pdfOcrGetAsyncJobStatus($async_job_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling PdfOcrApi->pdfOcrGetAsyncJobStatus: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
# **pdfOcrPost**
> \Swagger\Client\Model\PdfToTextResponse pdfOcrPost($image_file, $recognition_mode, $language, $preprocessing)
