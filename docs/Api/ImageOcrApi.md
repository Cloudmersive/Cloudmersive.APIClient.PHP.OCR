...
> \Swagger\Client\Model\PhotoToWordsWithLocationResult imageOcrPhotoWordsWithLocation($image_file, $language, $preprocessing)
...
try {
    $result = $apiInstance->imageOcrPhotoWordsWithLocation($image_file, $language, $preprocessing);
    print_r($result);
} catch (Exception $e) {
