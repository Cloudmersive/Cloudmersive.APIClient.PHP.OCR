...
> \Swagger\Client\Model\ImageToLinesWithLocationResult imageOcrImageLinesWithLocation($image_file, $language)
...
> \Swagger\Client\Model\ImageToWordsWithLocationResult imageOcrImageWordsWithLocation($image_file, $language)
...
try {
    $result = $apiInstance->imageOcrImageWordsWithLocation($image_file, $language);
    print_r($result);
} catch (Exception $e) {
