...
> \Swagger\Client\Model\ReceiptRecognitionResult imageOcrPhotoRecognizeReceipt($image_file, $language)
...
try {
    $result = $apiInstance->imageOcrPhotoRecognizeReceipt($image_file, $language);
    print_r($result);
} catch (Exception $e) {
