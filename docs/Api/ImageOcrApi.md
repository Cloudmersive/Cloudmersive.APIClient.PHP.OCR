...
> \Swagger\Client\Model\PhotoToWordsWithLocationResult imageOcrPhotoWordsWithLocation($image_file, $language)
...
try {
    $result = $apiInstance->imageOcrPhotoWordsWithLocation($image_file, $language);
    print_r($result);
} catch (Exception $e) {
